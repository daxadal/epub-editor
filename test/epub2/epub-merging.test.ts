...
describe('EPUB Merging', () => {
  ...
  describe('Basic Merging', () => {
    it('Two EPUBs are merged with all chapters preserved', async () => {
      ...
      const epub1 = await EPUB2Builder.parse(book1Path);
      const epub2 = await EPUB2Builder.parse(book2Path);

      // when
      const mergedEpub = new EPUB2Builder({
    ...
    it('Three EPUBs are merged into a single book', async () => {
      ...
      const sourceEPUBs = await Promise.all([
        EPUB2Builder.parse(book1Path),
        EPUB2Builder.parse(book2Path),
        EPUB2Builder.parse(book3Path),
      ]);
  ...
  describe('Export Merged EPUB', () => {
    it('Merged EPUB is exported successfully with valid structure', async () => {
      ...
      const parsedMerged = await EPUB2Builder.parse(outputPath);
